...
export default function ChatPage() {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 0,
      type: 'ai',
      content: "Bonjour ! Décrivez-moi ce que vous devez faire et je structure vos tâches automatiquement. Vous validez avant qu'elles soient ajoutées.\n\nEx: \"Je dois préparer la présentation client pour vendredi, relire le contrat avant jeudi\"",
    }
  ])
  ...
  async function handleValidate(msgId: number, task: ParsedTask) {
    await createTask({
      title: task.title,
      ...
      <div className="flex-1 overflow-y-auto py-6 flex flex-col gap-4">
        {messages.map(msg => (
          <div key={msg.id} className={`flex gap-3 ${msg.type === 'user' ? 'flex-row-reverse' : ''}`}>
            ...
            <div className={`flex flex-col gap-2 ${msg.type === 'user' ? 'items-end' : 'items-start'} flex-1`}>
              ...
              {msg.tasks && msg.tasks.length > 0 && (
                <div className="flex flex-col gap-2 w-full max-w-lg">
                  ...
                  {msg.tasks.length > 1 && !validated.has(msg.id) && (
                    ...
                    >
                      ✓ Valider toutes les tâches ({msg.tasks.length})
                    </button>
                  )}
                  ...
                  {msg.tasks.map((task, i) => (
                    <div key={i} className="bg-[#141417] border border-[#7c6af7]/40 rounded-xl p-4">
                      ...
                      {validated.has(msg.id) ? (
                        <div className="text-center text-xs text-[#6af7a0] py-1">✓ Ajoutée au dashboard</div>
                      ) : (
        ...
        {loading && (
          <div className="flex gap-3">
            <div className="w-8 h-8 rounded-lg bg-[#1c1c21] border border-[#2a2a32] flex items-center justify-center text-[#7c6af7] font-bold text-sm flex-shrink-0">✦</div>
            <div className="bg-[#141417] border border-[#2a2a32] rounded-xl px-4 py-3 flex gap-1.5 items-center">
              {[0, 1, 2].map(i => (
                <div key={i} className="w-1.5 h-1.5 bg-[#7c6af7] rounded-full animate-bounce"
      ...
      <div className="py-4 border-t border-[#2a2a32]">
        <div className="flex gap-2 items-end bg-[#1c1c21] border border-[#2a2a32] rounded-xl px-4 py-3 focus-within:border-[#7c6af7] transition-colors">
          <textarea
            ...
            placeholder="Décrivez vos tâches en langage naturel..."
            rows={1}
        ...
        <p className="text-xs text-[#6b6b80] text-center mt-2">Entrée pour envoyer · Shift+Entrée pour nouvelle ligne</p>
      </div>
